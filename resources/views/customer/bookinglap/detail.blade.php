...
@section('content')
    <div class="container mx-auto p-5">
        <a href="{{ route('customer.bookinglap') }}"
            class="items-center text-indigo-600 mb-5 border border-indigo-600 rounded px-2 py-2 hover:bg-indigo-600 hover:text-white transition duration-200">
            ...
            <div class="flex flex-col md:flex-row">
                <div class="md:w-1/2">
                    <img src="{{ asset('storage/' . $pesanan->lapangan->foto) }}"
                        alt="{{ $pesanan->lapangan->nama_lapangan }}" class="w-full h-auto object-cover">
                </div>
                <div class="md:w-1/2 p-8">
                    ...
                    <p class="text-gray-700 mb-2"><strong>Harga/Jam:</strong>
                        Rp{{ number_format($pesanan->lapangan->harga_per_jam, 0, ',', '.') }}</p>
                    ...
                    <p class="text-gray-700 mb-2"><strong>Jam:</strong> {{ implode(', ', explode(',', $pesanan->jam)) }}
                        WIB</p>
                    <p class="text-gray-700 mb-4"><strong>Total Bayar:</strong>
                    ...
                    <div class="flex gap-3 mt-4">
                        ...
                        @if ($pesanan->status == 'pending')
                            <a href="/customer/bookinglap/{{ $pesanan->id }}/payment"
                        ...
                        <button type="button" class="bg-red-500 text-white py-2 px-4 rounded hover:bg-red-600"
                            onclick="openModal()">
                            Batalkan Booking
                        </button>
                    ...
                    <div id="confirmationModal"
                        class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center transition-opacity duration-300">
                        <div class="bg-white rounded-lg p-6 max-w-sm mx-auto">
                            ...
                            <div class="flex justify-end">
                                ...
                                <form action="{{ route('customer.bookinglap.cancel', $pesanan->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="bg-red-500 text-white px-4 py-2 rounded">Ya,
                                        Batalkan</button>
                                </form>
                            </div>
                        </div>
